...
export default function Header() {
  ...
  const [isSearchOpen, setIsSearchOpen] = useState(false); // Arama kutusu durumu
  const [searchQuery, setSearchQuery] = useState(""); // Arama metni
  
  const menuRef = useRef<HTMLDivElement>(null);
  const searchInputRef = useRef<HTMLInputElement>(null); // Input focus için
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      ...
      setIsSearchOpen(false); // Aramadan sonra kapat
    }
  };
  ...
  useEffect(() => {
    setIsMenuOpen(false);
    setIsSearchOpen(false); // Sayfa değişince aramayı da kapat
  }, [pathname]);
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 transition-all">
      <div className="container mx-auto px-4 h-16 flex justify-between items-center relative">
        ...
        {/* NAVIGASYON (Orta) - Arama açılınca mobilde gizlenir */}
        ...
        <div className="flex items-center gap-1 md:gap-3 justify-end">
          ...
          <div className="flex items-center gap-1">
            ...
            <form onSubmit={handleSearchSubmit} className={`relative flex items-center justify-end transition-all duration-500 ease-in-out ${isSearchOpen ? 'w-[calc(100vw-40px)] md:w-64' : 'w-9'}`}>
               ...
               />
               <button 
                 type="button" // Submit olmaması için button, toggle yapar
          ...
          {user ? (
             ...
             <div className={`relative transition-opacity duration-300 ${isSearchOpen ? 'hidden md:block' : 'block'}`} ref={menuRef}>
               ...
               >
                 {/* Avatar SOLDA */}
                 <div className={`w-9 h-9 rounded-full bg-[#db4c3f]/10 border border-transparent group-hover:border-[#db4c3f]/30 flex items-center justify-center text-[#db4c3f] transition ring-2 ring-transparent ${isMenuOpen ? 'ring-[#db4c3f]/20' : ''}`}>
